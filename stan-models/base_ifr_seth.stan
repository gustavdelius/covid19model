...
parameters {
  ...
  real logit_multiplier; // We will multiply ifr by 2 * inv_logit(logit_multiplier)
}
...
transformed parameters {
    ...
    real multiplier = 2 * inv_logit(logit_multiplier);
    // The IFR calculated by Imperial group will be multiplied by the random ifr_factor
    vector<lower = 0>[M] ifr_factor = ifr_noise * multiplier; 
    // Number of initial infecteds should be divided by multiplier,
    // hence tau ~ exponential(0.03) / multiplier
    real<lower = 0> tau = tau_unit / 0.03 / multiplier; 
