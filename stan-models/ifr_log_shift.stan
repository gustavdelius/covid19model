...
parameters {
  ...
  real<lower=-1,upper=2> log_ifr_factor;
...
transformed parameters {
  ...
  matrix[N2, M] Rt = exp(-(covariate1 * alpha[1]
                           ...
                           + covariate5 * alpha[5]
                           + covariate6 * alpha[6]));
  real ifr_factor = 10^log_ifr_factor;
  real<lower = 0> tau = tau_unit / 0.03 * ifr_factor;
  vector<lower = 0>[M] y = tau * y_unit; // y ~ exponential(1/tau)
  vector<lower = 0>[M] ifr = exp(ifr_std * 0.1) * ifr_factor;
