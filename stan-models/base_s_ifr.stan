...
parameters {
  ...
  real<lower = 0.4, upper = 1> s; // Proportion of population initially susceptible
}
...

    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          ...
          Rt_adj[i,m] = ((s * pop[m] - cumm_sum[i,m]) / pop[m]) * Rt[i,m];
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          ...
          prediction0[i, m] = ((s * pop[m] - cumm_sum0[i,m]) / pop[m]) * mu[m] * convolution0;
        }
