...
transformed parameters {
  ...
  matrix[N2, M] Rt_adj = Rt; // Effective R including immunity
  real<lower = 0> tau = tau_unit / 0.03; // tau ~ exponential(0.03)
  vector<lower = 0>[M] y = tau * y_unit; // y ~ exponential(1/tau)
  
  { // This is in a code block so that we can declare local variables
    ...
    for (m in 1:M) {
      ...
      for (i in (N0 + 1):N2) {
        ...
        // increment number of immune individuals
