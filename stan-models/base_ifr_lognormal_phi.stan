...
transformed parameters {
    ...
    // Number of initial infecteds should be divided by exp(mu)
...
model {
  ...
  mu ~ normal(0, 1);
  ifr_factor ~ lognormal(mu, 1);
  for(m in 1:M){
    deaths[EpidemicStart[m]:N[m], m] ~ neg_binomial_2(E_deaths[EpidemicStart[m]:N[m], m], phi[m]);
   }
}
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          ...
          prediction0[i, m] = ((pop[m]-cumm_sum0[i,m]) / pop[m]) * R0[m] * convolution0;
        }
