...
if(DEBUG) {
  fit = sampling(m,data=stan_data,iter=40,warmup=20,chains=2,
                 init = list(list(log_infecteds_multiplier = 3),
                             list(log_infecteds_multiplier = 2)))
} else if (FULL) {
  fit = sampling(m,data=stan_data,iter=1800,warmup=1000,chains=5,thin=1,control = list(adapt_delta = 0.95, max_treedepth = 15),
                 init = list(list(log_infecteds_multiplier = 3),
                             list(log_infecteds_multiplier = 2),
                             list(log_infecteds_multiplier = 1),
                             list(log_infecteds_multiplier = 0),
                             list(log_infecteds_multiplier = -1)))
} else { 
